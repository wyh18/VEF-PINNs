...
class VEF_PINN(object):
    ...
    def _load_reference_data(self):
        ...
        rho = torch.sin(2 * torch.pi * (self.ref_x + self.ref_t)) / (2 * torch.pi) + 2 - 1 / 3 * self.kn * torch.cos(
            2 * torch.pi * (self.ref_x + self.ref_t))
        self.ref_rho = rho.cpu()
    ...
    def _init_reference_grid(self):
        ...
        self.kn = self.Config["physical_config"]["kn"]
    ...
    def final_plot(self, model, model_type: str):
        ...
        with torch.no_grad():
            if model_type == "I":
                eqn = equation_I.LinearTransport(config=self.Config, sol=model)
                pred1 = eqn.rho(sol=model, inputs=(self.ref_t, self.ref_x)).cpu().numpy()
            elif model_type == "rho":
                pred1 = model(inputs).cpu().numpy()
        ...
        plt.plot(self.ref_x.cpu().detach().numpy(), pred1, color='b', marker='*', linestyle='--', linewidth=1.0,
                 ...
                 label='VEF-PINNs(t = 0)')
        plt.plot(self.ref_x.cpu().detach().numpy(), self.ref_rho.cpu().detach().numpy(), color='g', linewidth=1.0, markersize=10, label='Ref(t = 0)')
        ...
        error = torch.sqrt(
            torch.mean((self.ref_rho.cpu() - pred1) ** 2)
            / (torch.mean(self.ref_rho.cpu() ** 2) + 1e-15)
        ).numpy()
...
def main():
    ...
    # if eps = 1, choose model_I; else choose model_rho
    error_f = pinn.final_plot(pinn.model_I, "I")
    error_rho = pinn.final_plot(pinn.model_rho, "rho")

    print(f"Errors of model_I for t=0.1 : {error_f:.2e}")
    print(f"Errors of model_rho for t=0.1 : {error_rho:.2e}")
